...
- A tracking write in `apps/mobile` is a **value**, not a closure: add a case to `TrackingWrite` and to `runTrackingWrite`/`trackingPatch` in `src/lib/offline.ts`, and call it through `useViewerMutation` or a `useMutation({ mutationKey: TRACKING_MUTATION_KEY })`. A write React Query pauses offline is persisted as its key and its variables and nothing else, so anything that cannot round-trip through JSON is a check-in lost on the next launch. Anything a queued write must still do after it resumes — cache invalidation above all — belongs in the mutation defaults, not only in the hook.
