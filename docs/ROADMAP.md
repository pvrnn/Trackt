...
| Web          | Profile page + favourites (`/profile`, mockup Profile.dc.html): `GET /api/v1/me/profile`, ♥ favourite toggle on media pages, per-kind ranked shelves, stats strip, recent feed; ACTIVITY nav + account-menu Profile wired; followers/badges/visibility wait for v1.x | `apps/web/src/routes/profile.tsx`     |
...
- Lists (mockup: `Lists.dc.html`); profile follow-ups: edit profile (bio/avatar), favourite reordering, public profiles + visibility (v1.x)
